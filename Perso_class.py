class player(object):
    ...
    def __init__(self,name="Joueur",basexp=0,mage=False):
        ...
        self.thirdstats["invested_armor"]={"Heaume":0,"Spallières":0,"Brassards":0,"Avant-bras":0,"Plastron":0,"Jointures":0,"Tassette":0,"Cuissots":0,"Grèves":0,"Solerets":0}
        ...
        self.playerequipment={"Heaume":None,"Spallières":None,"Brassards":None,"Avant-bras":None,"Plastron":None,"Jointures":None,"Tassette":None,"Cuissots":None,"Grèves":None,"Solerets":None,"left_melee":None,"right_melee":None,"left_throw":None,"right_throw":None}
    ...
    def upstats(self,stat,number=0,location=None):
        ...
        if stat in self.basestats.keys() and number<=self.xp:
            if (self.basestats[stat][0]+number <= self.basestats[stat][1]) and (self.basestats[stat][0]+number>=0):
                self.basestats[stat][0]+=number
                self.xp-=number
        
        elif stat =="phys-res"and number<=(self.secondstats["symb-strength"][2]+self.secondstats["symb-strength"][0]):
            if (self.thirdstats[stat][0]+number <= self.thirdstats[stat][1]) and (self.thirdstats[stat][0]+number>=0):
        ...
        # Pour ce tier 3, "number" sera probablement égal à 1
            ...
            if self.secondstats["symb-stealth"][0]+self.secondstats["symb-stealth"][1]+self.secondstats["symb-T"]>=number:
            ...
            if self.secondstats["symb-ability"][0]+self.secondstats["symb-ps_T"][0]+self.secondstats["symb-T"]>=number:
    ...
    def calculate(self,stat):
        ...
        if stat in ["hands","light","medium","heavy","throw","shield","training"]:
            possible_strength=self.basestats["hands"][0]//5+self.basestats["light"][0]//10+self.basestats["medium"][0]//10+self.basestats["heavy"][0]//10+self.basestats["throw"][0]//10+self.basestats["shield"][0]//10
            possible_strength+=min(10,self.basestats["training"][0]//2)+min(10,max(0,self.basestats["training"][0]-40)//2)+min(10,max(0,self.basestats["training"][0]-80)//2)+min(10,max(0,self.basestats["training"][0]-120)//2)+min(10,max(0,self.basestats["training"][0]-160)//2)
            possible_strength+=5*((self.basestats["training"][0]>=30)+(self.basestats["training"][0]>=70)+2*(self.basestats["training"][0]>=110)+2*(self.basestats["training"][0]>=150))
            ...
            if not stat in ["shield","training"]:
                self.secondstats["symb-mastery"][stat]=5*((self.basestats[stat][0]>=90)+(self.basestats[stat][0]>=110)+(self.basestats[stat][0]>=160)+(self.basestats[stat][0]>=170))
                self.secondstats["symb-mastery"][stat]+=self.GMstats[stat]
                
            
            elif stat == "shield":
                self.secondstats["symb-parry"]=5*((self.basestats[stat][0]>=60)+(self.basestats[stat][0]>=90)+(self.basestats[stat][0]>=110)+(self.basestats[stat][0]>=160)+(self.basestats[stat][0]>=170))
                self.secondstats["symb-parry"]+=self.GMstats["symb-parry"]
        
        elif stat =="phys-res":
            self.secondstats["symb-strength"][1]=5+5*min(4,self.thirdstats[stat][0]//20)+5*min(4,max(0,self.thirdstats[stat][0]-90)//20)
            
            possible_strength=self.secondstats["symb-strength"][0]+self.secondstats["symb-strength"][2]+self.GMstats["symb-strength"]
            self.secondstats["symb-strength"][0]=min(possible_strength,self.secondstats["symb-strength"][1])
            self.secondstats["symb-strength"][2]=max(0,possible_strength-self.secondstats["symb-strength"][1])
            ...
            # symbole d'armure
            self.secondstats["symb-armor"]=5*((self.basestats[stat][0]>=40)+(self.basestats[stat][0]>=60)+(self.basestats[stat][0]>=70)+(self.basestats[stat][0]>=110)+(self.basestats[stat][0]>=120)+(self.basestats[stat][0]>=130)+(self.basestats[stat][0]>=170))
            ...
            # niveau d'armure
            self.secondstats["armor-level"]=((self.basestats[stat][0]>=10)+(self.basestats[stat][0]>=20)+(self.basestats[stat][0]>=30)+(self.basestats[stat][0]>=50)+(self.basestats[stat][0]>=80)+(self.basestats[stat][0]>=100)+(self.basestats[stat][0]>=150)+(self.basestats[stat][0]>=180))
            
        
        elif stat == "dexterity":
            # symbole
            self.secondstats["symb-ability"][0]=min(30,self.basestats[stat][0]//2)+min(10,max(0,self.basestats[stat][0]-60)//4)+min(8,max(0,self.basestats[stat][0]-100)//5)+min(10,max(0,self.basestats[stat][0]-140)//4)
            self.secondstats["symb-ability"][0]+=((self.basestats[stat][0]>=90)+(self.basestats[stat][0]>=110)+(self.basestats[stat][0]>=120)+(self.basestats[stat][0]>=140)+(self.basestats[stat][0]>=150)+(self.basestats[stat][0]>=160)+(self.basestats[stat][0]>=170))
            ...
            #symbole
            self.secondstats["symb-mobility"]=min(4,self.basestats[stat][0]//5)+min(10,max(0,self.basestats[stat][0]-30)//5)+min(5,max(0,self.basestats[stat][0]-80)//4)+min(7,max(0,self.basestats[stat][0]-105)//5)+min(5,max(0,self.basestats[stat][0]-160)//4)
            self.secondstats["symb-mobility"]+=5*((self.basestats[stat][0]>=30)+(self.basestats[stat][0]>=90)+(self.basestats[stat][0]>=105)+(self.basestats[stat][0]>=120)+(self.basestats[stat][0]>=145)+(self.basestats[stat][0]>=150))
            self.secondstats["symb-mobility"]+=self.GMstats["symb-mobility"]
        
        elif stat == "perception":
            # symbole
            self.secondstats["symb-perception"]=5*((self.basestats[stat][0]>=30)+(self.basestats[stat][0]>=40)+(self.basestats[stat][0]>=60)+(self.basestats[stat][0]>=70)+(self.basestats[stat][0]>=80)+(self.basestats[stat][0]>=115)+(self.basestats[stat][0]>=125)+2*(self.basestats[stat][0]>=180)+3*(self.basestats[stat][0]>=190))
            ...
            #symbole S
            self.secondstats["symb-S"]=5*((self.basestats[stat][0]>=25)+(self.basestats[stat][0]>=65)+(self.basestats[stat][0]>=75)+(self.basestats[stat][0]>=85)+(self.basestats[stat][0]>=90)+(self.basestats[stat][0]>=105)+(self.basestats[stat][0]>=160)+(self.basestats[stat][0]>=170))
            self.secondstats["symb-S"]+=self.basestats[stat][0]//5
            self.secondstats["symb-S"]+=5*((self.basestats["sensitivity"][0]>=90)+(self.basestats["sensitivity"][0]>=110)+(self.basestats["sensitivity"][0]>=160)+(self.basestats["sensitivity"][0]>=170))
            self.secondstats["symb-S"]+=self.GMstats["symb-S"]
            self.secondstats["symb-S"]-=(self.thirdstats["sight"]+self.thirdstats["hearing"]+self.thirdstats["smell"])
            
        elif stat in ["reflex","stealth"]:
            # symbole T
            self.secondstats["symb-T"]=5*((self.basestats["reflex"][0]>=90)+(self.basestats["reflex"][0]>=110)+(self.basestats["reflex"][0]>=160)+(self.basestats["reflex"][0]>=170)) #- ce qui a été consommé (T et S), + la magie, +les autres carac
            self.secondstats["symb-T"]+=(5*((self.basestats["stealth"][0]>=25)+(self.basestats["stealth"][0]>=65)+(self.basestats["stealth"][0]>=75)+(self.basestats["stealth"][0]>=85)+(self.basestats["stealth"][0]>=90)+(self.basestats["stealth"][0]>=105)+(self.basestats["stealth"][0]>=160)+(self.basestats["stealth"][0]>=170)))
            ...
            if stat == "stealth":
                self.secondstats["symb-stealth"][0]=5*((self.basestats[stat][0]>=30)+(self.basestats[stat][0]>=40)+(self.basestats[stat][0]>=60)+(self.basestats[stat][0]>=70)+(self.basestats[stat][0]>=80)+(self.basestats[stat][0]>=115)+(self.basestats[stat][0]>=125)+2*(self.basestats[stat][0]>=180)+3*(self.basestats[stat][0]>=190))
            ...
            if stat == "reflex":
                self.secondstats["symb-init"]=5*((self.basestats[stat][0]>=30)+(self.basestats[stat][0]>=40)+(self.basestats[stat][0]>=60)+(self.basestats[stat][0]>=70)+(self.basestats[stat][0]>=80)+(self.basestats[stat][0]>=120)+2*(self.basestats[stat][0]>=180)+3*(self.basestats[stat][0]>=190))
                self.secondstats["symb-init"]+=(min(5,self.basestats[stat][0]//4)+min(5,max(0,self.basestats[stat][0]-40)//4)+min(5,max(0,self.basestats[stat][0]-130)//4))
                ...
                self.secondstats["symb-ps_T"][0]+=(self.secondstats["symb-ps_T"][1]+self.secondstats["symb-ps_T"][2])
            ...
            # symbole
            self.secondstats["symb-light"]=min(15,max(0,self.basestats[stat][0]-10)//2)+min(25,max(0,self.basestats[stat][0]-50)//2)+min(20,max(0,self.basestats[stat][0]-110)//2)+min(15,max(0,self.basestats[stat][0]-160)//2)
            self.secondstats["symb-light"]+=self.GMstats["symb-light"]
            
            
        elif stat == "mental-res":
            # symbole
            self.secondstats["symb-mental"]=min(20,max(0,self.basestats[stat][0]-10)//2)+min(10,max(0,self.basestats[stat][0]-60)//4)+min(10,max(0,self.basestats[stat][0]-110)//4)+min(8,max(0,self.basestats[stat][0]-158)//4)
            self.secondstats["symb-mental"]+=self.GMstats["symb-mental"]
            
            
        elif stat == "trading":
            # symbole 3 cubes empilés
            self.secondstats["symb-trading"]=min(40,max(0,self.basestats[stat][0]-10)//2)+min(40,max(10,self.basestats[stat][0]-100)//2)+min(40,max(15,self.basestats[stat][0]-130)//2)+min(40,max(10,self.basestats[stat][0]-160)//3)
            self.secondstats["symb-trading"]+= 10*((self.basestats[stat][0]>=170)+(self.basestats[stat][0]>=190))
            self.secondstats["symb-trading"]+= self.GMstats["symb-trading"]
        
        
        elif stat in ["power","mastery"]:
            # symbole
            self.secondstats["symb-lightning"]=min(10,self.basestats["power"][0]//2)+min(10,max(0,self.basestats["power"][0]-20)//3)+min(25,max(0,self.basestats["power"][0]-60)//4)
            self.secondstats["symb-lightning"]+=(min(20,self.basestats["mastery"][0])+min(15,max(0,self.basestats["mastery"][0]-20)//2)+min(40,max(0,self.basestats["power"][0]-60)//2)+min(10,max(0,self.basestats["power"][0]-150)//4))
            self.secondstats["symb-lightning"]+=10*((self.basestats["power"][0]>=170)+2*(self.basestats["power"][0]>=190)+(self.basestats["mastery"][0]>=90))
            ...
            # symbole
            self.secondstats["symb-sensi"]=10*((self.basestats[stat][0]>=30)+(self.basestats[stat][0]>=40)+(self.basestats[stat][0]>=60)+(self.basestats[stat][0]>=70)+(self.basestats[stat][0]>=80)+(self.basestats[stat][0]>=100)+(self.basestats[stat][0]>=120)+(self.basestats[stat][0]>=150)+2*(self.basestats[stat][0]>=180)+2*(self.basestats[stat][0]>=190))
            self.secondstats["symb-sensi"]+=self.GMstats["symb-sensi"]
            
            # symbole S
            self.secondstats["symb-S"]=5*((self.basestats[stat][0]>=90)+(self.basestats[stat][0]>=110)+(self.basestats[stat][0]>=160)+(self.basestats[stat][0]>=170))
            self.secondstats["symb-S"]+=5*((self.basestats["perception"][0]>=25)+(self.basestats["perception"][0]>=65)+(self.basestats["perception"][0]>=75)+(self.basestats["perception"][0]>=85)+(self.basestats["perception"][0]>=90)+(self.basestats["perception"][0]>=105)+(self.basestats["perception"][0]>=160)+(self.basestats["perception"][0]>=170))
            ...
            # symbole
            self.secondstats["symb-aura"]=min(20,self.secondstats[stat][0])+min(20,max(0,self.secondstats[stat][0]-20)//2)+min(20,max(0,self.secondstats[stat][0]-60)//4)+min(50,max(0,self.secondstats[stat][0]-140))
    ...
    def calc_perc(self,stat):
        ...
        if stat in ["hands","light","medium","heavy","throw","shield","charisma","trading","power"]:
            ...
            if stat=="power":
                self.percentages["spell"]=min(20,self.basestats[stat][0])+5*(self.basestats[stat][0]>=50)+min(4,max(0,self.basestats[stat][0]-130)//5)
            else:
                self.percentages[stat]=min(20,self.basestats[stat][0])+5*(self.basestats[stat][0]>=50)+min(4,max(0,self.basestats[stat][0]-130)//5)
        
        elif stat in["mobility","perception","stealth","sensitivity"]:
            self.percentages[stat]=min(20,self.basestats[stat][0])+min(10,max(0,self.basestats[stat][0]-40)//2)+min(4,max(0,self.basestats[stat][0]-130)//5)
        
        elif stat == "dexterity":
            self.percentages[stat]=min(10,self.basestats[stat][0]//2)+min(5,max(0,self.basestats[stat][0]-20)//4)
            self.percentages[stat]+=5*((self.basestats[stat][0]>=50)+(self.basestats[stat][0]>=70))
        
        elif stat == "phys-res":
            self.percentages[stat]=min(20,self.thirdstats[stat][0])+min(10,max(0,self.thirdstats[stat][0]-30)//2)+min(5,max(0,self.thirdstats[stat][0]-80)//4)+min(5,max(0,self.thirdstats[stat][0]-110)//4)+min(5,max(0,self.thirdstats[stat][0]-140)//4)
            self.percentages[stat]+=5*(self.thirdstats[stat][0]>=70)
        
        elif stat == "mental-res":
            self.percentages[stat]=min(20,self.basestats[stat][0])+min(10,max(0,self.basestats[stat][0]-30)//2)+min(4,max(0,self.basestats[stat][0]-70)//5)+min(8,max(0,self.basestats[stat][0]-110)//5)
            ...
            if type(obj)==ArmorEquip:
                ...
                if obj.carac["hand"]==1:
                    if self.playerequipment[where+"_throw"]:
                        ...
                        if self.playerequipment["right"*(where=="left")+"left"*(where=="right")+"_throw"]==obj and self.inventory[obj]<=1:
                            self.playerequipment["right"*(where=="left")+"left"*(where=="right")+"_throw"]=None
                ...
                if obj.carac["hand"]==1:
                    if self.playerequipment[where+"_melee"]:
                        ...
                        if self.playerequipment["right"*(where=="left")+"left"*(where=="right")+"_melee"]==obj and self.inventory[obj]<=1:
                            self.playerequipment["right"*(where=="left")+"left"*(where=="right")+"_melee"]=self.playerequipment[where+"_melee"]
                    
                    else:
                        if self.playerequipment["right"*(where=="left")+"left"*(where=="right")+"_melee"]==obj and self.inventory[obj]<=1:
                            self.playerequipment["right"*(where=="left")+"left"*(where=="right")+"_melee"]=None
